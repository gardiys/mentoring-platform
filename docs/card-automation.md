...
### Переиспользование ответов из разбора

Автопубликация использует `suggested_better_answer` как **непроверенный черновик**,
а не как доверенный источник. Создание кластера и пересчёт статистики сохраняют
такой ответ даже без ссылок на материалы. Если черновик найден позднее, задание
генерации переносит его без обращения к генератору и ставит независимую проверку.
Восстановление очереди также отправляет сохранённый черновик без ссылок и без
актуальной проверки на валидацию, не расходуя лимит исправлений. Выявленные
проверкой ошибки и ссылки на недоступные материалы по-прежнему требуют доработки.

Проверяющая модель оценивает полноту ответа на самостоятельный вопрос и возвращает
`supporting_source_references` — идентификаторы конкретных переданных материалов,
которые подтверждают текст. Только успешная проверка позволяет дополнить ими
черновик. Непереданные или отсутствующие ссылки, противоречия, неполный ответ и
неподтверждённый личный опыт не разрешают публикацию. Пороги уверенности,
предварительные проверки, права на источники и контроль дублей сохранены.

Если ответ неполный или содержит советы конкретному ученику, существующий механизм
исправлений получает предыдущий текст и замечания. Исправленный ответ снова
проходит независимую проверку; лимит — два исправления. Сам факт переноса из
AI-разбора записывается в `source_limitations`, а не выдаётся за фактическую ошибку.
При отсутствии материалов готовый ответ сохраняется до их появления.

В техническом разборе рекомендации ученику отделены от `suggested_better_answer`:
если исправление нужно, это поле должно содержать компактный самостоятельный ответ
со всеми необходимыми частями, без персонального фидбека. Для уже полного правильного
ответа поле по-прежнему может быть пустым. В таком случае генерация общей карточки
может понадобиться.

При однозначном точном совпадении вопроса с опубликованной карточкой того же
направления разбор получает её ответ как общий ориентир, отдельно от речи ученика.
Сравнение игнорирует только регистр и пробелы по краям; неоднозначные, скрытые,
неопубликованные и относящиеся к другому направлению карточки не используются.
Верные альтернативные ответы допустимы, а знания из эталона нельзя приписывать
ученику. Идентификатор и текст эталона входят в ключ checkpoint: изменение ответа
не позволяет повторно использовать устаревший разбор при его новом запуске.

Полный подходящий черновик экономит вызов генератора. Неполный может потребовать
проверки, исправления и повторной проверки; экономия не гарантируется для каждого
случая. Отсутствие подтверждения материалами в UI отделено от противоречий,
недостаточной полноты ответа и нехватки контекста вопроса.

Изменение не требует миграции. Обновлять backend, оба intelligence-worker и frontend
нужно согласованно: JSON результата проверки получил новое поле. Версии промптов
и схемы проверки изменены, чтобы новые проверки не использовали старый кэш.
Существующие ответы и ручные решения не сбрасываются, массовый перезапуск не выполняется.
