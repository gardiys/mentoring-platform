...
Формирование, версионирование и юридически значимое предоставление Карьерных пакетов описано в
[`docs/career-packages.md`](docs/career-packages.md).
...
Для FirstVDS используйте API endpoint `https://firsts3.ru` и регион `default`. Провайдер [сменил домен S3](https://firstvds.ru/blog/vazhnoe-obnovlenie-s3-pereezzhaet-na-novyy-domen-firstvds); старый `s3.firstvds.ru` больше не разрешается в DNS. Укажите новый адрес в `S3_ENDPOINT_URL` и `S3_PUBLIC_ENDPOINT_URL`: первый доступен backend-контейнеру, второй должен открываться из браузера. Для совместимости приложение нормализует старый HTTPS endpoint перед созданием S3-клиентов и подписей. Архивные ссылки в БД менять не требуется: bucket и ключ сохраняются, а доступ подписывается на новом домене. Архивные записи из bucket `interviews` также читаются через авторизованный S3 API: личный дневник получает временный presigned URL, а защищённый URL каталога после проверки playback-ticket перенаправляет браузер на ещё более короткую подписанную ссылку. Прямой URL объекта в private bucket без `X-Amz-*` параметров должен отвечать `403` — это ожидаемое поведение.
...
Для собеседований используется `diarization_setting=telephonic`, рассчитанный на перебивания
и одновременную речь. Количество говорящих определяется автоматически.

AI-извлечение вопросов и оценка ответов получают небольшой словарь терминов выбранного
направления (Python/Go) и правила объединения ответа кандидата через перебивания интервьюеров.
Это контекст для анализа готовой расшифровки, а не словарь распознавания Nexara.
Предложения по написанию терминов сохраняются отдельно в `transcription_annotations` вопроса
и видны рядом с ответом. Backend проверяет исходную реплику, вариант из словаря и уверенность
не ниже 0.95; неуверенные фрагменты помечаются для проверки. Исходная расшифровка и текст
ответа не перезаписываются. Для хранения подсказок нужна миграция `20260908_0083`.
Изменения применяются к новым AI-разборам, старые результаты автоматически не пересчитываются.
...
Расходы всех прямых OpenAI-запросов, включая embeddings backfill и невалидные ответы,
теперь отдельно записываются в `ai_request_logs` до проверки структуры ответа. Успешные
фрагменты извлечения, оценки ответов и промежуточные итоги сохраняются для продолжения
после сбоя. Настройка Flex для фоновой автоматизации, миграция и команда отчёта описаны в
[оптимизациях расходов AI](docs/ai-cost-optimizations.md).
